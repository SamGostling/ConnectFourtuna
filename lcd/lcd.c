...
//Method added to allow indivdual pixels to be drawn
void fill_pixel(uint16_t x, uint16_t y, uint16_t colour)
{
    write_cmd(COLUMN_ADDRESS_SET);
    write_data16(x);
    write_data16(x);
...
void draw_circle(int16_t  x0, int16_t  y0, uint16_t col, int16_t  radius)
{
    int16_t f = 1 - radius;
    int16_t ddF_x = 1;
    ...
    fill_pixel(x0, y0 + radius, col);
    fill_pixel(x0, y0 - radius, col);
    fill_pixel(x0 + radius, y0, col);
    fill_pixel(x0 - radius, y0, col);
    draw_line(x0 - radius, y0, x0 + radius, y0, col);
